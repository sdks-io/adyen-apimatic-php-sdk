...
$donationsApi = $client->getDonationsApi();
$apiResponse = $donationsApi->postDonationCampaigns(
    null,
    $body
);
...
```php
function postDonations(?string $idempotencyKey = null, ?DonationPaymentRequest $body = null): ApiResponse
```
...
$donationsApi = $client->getDonationsApi();
$apiResponse = $donationsApi->postDonations(
    null,
    $body
);
