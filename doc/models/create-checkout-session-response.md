...
| `dateOfBirth` | `?DateTime` | Optional | The shopper's date of birth in [ISO8601](https://www.iso.org/iso-8601-date-and-time-format.html) format. | getDateOfBirth(): ?\DateTime | setDateOfBirth(?\DateTime dateOfBirth): void |
...
| `id` | `string` | Required | A unique identifier of the session. | getId(): string | setId(string id): void |
...
| `sessionData` | `?string` | Optional | The payment session data you need to pass to your front end. | getSessionData(): ?string | setSessionData(?string sessionData): void |
...
| `url` | `?string` | Optional | The URL for the Hosted Checkout page. Redirect the shopper to this URL so they can make the payment. | getUrl(): ?string | setUrl(?string url): void |
